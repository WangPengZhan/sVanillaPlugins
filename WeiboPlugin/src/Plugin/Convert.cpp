...
adapter::VideoView convertVideoView(const weiboapi::ComponentPlayPlayinfoResponse& data)
{
    ...
    baseView.Identifier = data.data.id;
    baseView.AlternateId = data.data.mid;
    baseView.VideoId = std::to_string(data.data.media_id);
    baseView.Title = data.data.title;
    baseView.Publisher = data.data.author;
    baseView.Cover = "https:" + data.data.cover_image;
    baseView.Duration = formatDuration(data.data.duration_time);
    baseView.Description = data.data.text;
    baseView.PublishDate = convertTimestamp(data.data.real_date);
    baseView.PlayListTitle = data.data.title;
