...
std::shared_ptr<download::FileDownloader> WeiboPlugin::getDownloader(const VideoInfoFull& videoInfo)
{
    ...
    std::list<std::string> h = {std::string("User-Agent: ") + network::chrome};
    ...
    auto weiboDownlaoder = std::shared_ptr<download::WeiboDownloader>(new download::WeiboDownloader(), download::freeDownload);
