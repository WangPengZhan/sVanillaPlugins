...
class DedaoDownloader : public FileDownloader
{
public:
    DedaoDownloader();
    DedaoDownloader(std::list<std::string> videoUris, std::list<std::string> audioUri = {}, std::string path = "", std::string filename = "");
    explicit DedaoDownloader(ResourceInfo info);
    ~DedaoDownloader() = default;
