...
class DouYinDownloader : public FileDownloader
{
public:
    DouYinDownloader();
    DouYinDownloader(std::list<std::string> videoUris, std::list<std::string> audioUri = {}, std::string path = "", std::string filename = "");
    explicit DouYinDownloader(ResourceInfo info);
    ~DouYinDownloader() = default;
